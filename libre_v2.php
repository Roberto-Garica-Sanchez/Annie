<?php
#SELECT * FROM empresa.folio  JOIN empresa.abo_acu ON folio.ID_G=abo_acu.ID_G WHERE folio.ID_G='4004'
	/*
	...
	*/
	...
	if((!empty($_POST['js'])<>1)or($libre_v=="2"))	{
	...
	public 	function GeneraSql($array){
		...
		*/	
		...
		if(!empty($array['Operacion']['SELECT']['Activar']) and $array['Operacion']['SELECT']['Activar']=='true'){
			...
			$Condiciones='';
			#Gener las columnas que se descargaran 
			...
			#Genera Condiciones de ordenado de datos 		
			if(!empty($datos['ByOrder']) and !empty($datos['ByOrder']['Columna']))$Condiciones=$Condiciones." Order By ".$datos['ByOrder']['Columna'].' '.$datos['ByOrder']['ASC-DESC'];


			$sql="SELECT ".$getColumnas." FROM ".$array['tabla']." ".$Condiciones;
			$this->sql=$sql;
		}
...
class libre_v2	{		
	...
	function despliegre_mysql	($name,$name2,$consu,$descarga,$phpv,$libre,$className,$dataset,$set){
		...
		if(!empty($name2))$res=$res."<OPTION value='$name'>$name2</OPTION>";
			...
			$EXISTE_OPCION='false';
			while($datos= $this->mysql_fe_ar($consu,$phpv,'')){$set='';	
				if($_POST[$name]==$datos[$descarga]){
					$EXISTE_OPCION='true';
				}
	...
	function menu				($type,$style,$libre,$name,$v1,$v2,$v3,$v4,$v5,$v6,$v7,$v8,$v9,$v10,$v11,$script_input)			{	
		...
		$conte=$this->libre_v1->input2('hidden',$name,'',$_POST[$name],'','','','');
										//($type2,$name,$title,$value,$style,$id,$libre,$class)
		
		if(!empty($v1))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v1,"",$d1 ,''.$script_input,'');
		if(!empty($v2))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v2,"",$d2 ,''.$script_input,'');
		if(!empty($v3))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v3,"",$d3 ,''.$script_input,'');
		if(!empty($v4))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v4,"",$d4 ,''.$script_input,'');
		if(!empty($v5))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v5,"",$d5 ,''.$script_input,'');
		if(!empty($v6))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v6,"",$d6 ,''.$script_input,'');
		if(!empty($v7))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v7,"",$d7 ,''.$script_input,'');
		if(!empty($v8))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v8,"",$d8 ,''.$script_input,'');
		if(!empty($v9))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v9,"",$d9 ,''.$script_input,'');
		if(!empty($v10))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v10,"",$d10);
		if(!empty($v11))$conte=$conte.	$this->libre_v1->input2($type,$name,'',$v11,"",$d11);
...
class tablas_v2	{		
	...
	function info($db,$tb){//tablas_v2::info(empresa,folio);
		...
		if($db==$db1){
			...
			if($tb==$tb8){//diesel
				$traducion="auto";
				$array_mysql	= array(ID_G	,HIDE3,TOTAL3,"3TEXT1","3TEXT2","3TEXT3","3TEXT4","3TEXT5","3TEXT6","3TEXT7",presio_d,medidor_inicio,medidor_final);
				$array_none		= array('','','','','','','','','','','','','');//13
				$array_insert	= array('','','','','','','','','','','','','');//13	
				$array_update	= array('','','','','','','','','','','','','');//13	
				$array_name 	= array(Carta1	,HIDE3,TOTAL3,"3TEXT1","3TEXT2","3TEXT3","3TEXT4","3TEXT5","3TEXT6","3TEXT7",presio_d,crome_i,crome_f);
				$datos_encontrados=true;
