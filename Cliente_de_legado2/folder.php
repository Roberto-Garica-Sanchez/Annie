...
if($revi==0){$d15=input(submit,CambRevi,Pendiente,'','color: white; background: #343434;');}
if($revi==1){$d15=input(submit,CambRevi,Revisado,'','color: white; background: #343434;');}
...
if($_POST[dele_cue]==Eliminar){
	$i15=input(submit,confi_dele,Confirmar,'Este Proseso Es Ireversible Eliminando la Cuenta','color: red; background: #343434;');
		...
		$i17='Folder "Activo"';
		$d17=input2(submit,transferir,'Esto Cambiara El Chofer a Estado Inactivo',Suspender);
...
//--------------------------------------------------------
if ($_POST[chofer]<>Choferes){
$b1=input(button,'',N°		,'','width: 50px;');
$b2=input(button,'',Cuente	,'','width: 60px;');
$b3=input(button,'',Cliente ,'','width: 180px;');
$b4=input(button,'',Saldo	,'','width: 70px;');
$b5=input(button,'',Sueldo);
...
";
		...
		while($datos=mysql_fetch_array($consu5)){
			if (($_POST[chofer]==$datos[CHOFER])or($_POST[chofer]==chofer)){
				...
				$b1=input(button,'',$x	,''				,"width: 50px;");
				$b2=input(submit,Carta,$datos[ID_G]		,''				," width: 60px;","id='botones'");
				$b3=input(button,'',$datos[CLIENTE]		,$datos[CLIENTE],"background: $c1; color:$c2; width: 180px; text-align: left;");
				$b4=input(button,'',$dato[Total_Total]	,''				,"background: $c1; color:$c2; width: 70px;");
