<?php
	...
	/*if($_POST[win_carga]==reportes)	{
		libre_v1::db						(empresa,$conexion,$phpv);
		$consu5	= libre_v1::consulta		(folio,$conexion,CHOFER,$_POST[chofer],'1','',$phpv,'');
		libre_v1::mysql_da_se				($consu5,0,$phpv);			
		$choferes=array();
		$ch_value=array();
		...
		while($datos=libre_v1::mysql_fe_ar	($consu5,$phpv)){
			...
			$consi=0;
			for($x=0; $x<=$tam; $x++){
				if($choferes[$x]==$datos[CHOFER]){$consi=1;}//revisas si existe el nuevo dato 
			}
			if($consi==0){$choferes[] = $datos[CHOFER];}//agrega un nuevo campo 
			for($x=0; $x<=$tam; $x++){
				if($choferes[$x]==$datos[CHOFER]){$ch_value[$choferes[$x]] = $ch_value[$choferes[$x]]+$datos[sueldo];}// suma totales de sueldo 
			}
		}
		echo libre_v1::input2(text,'','',Chofer,'background: #363434; color: white; width: 200px;');
		echo libre_v1::input2(text,'','',Sueldo,'background: #363434; color: white; ');
		echo"<br>";
		for($x=0; $x<=$tam; $x++){
			
			echo libre_v1::input2(text,'','',$choferes[$x],'width: 200px;');
			echo libre_v1::input2(text,'','',$ch_value[$choferes[$x]]);
			echo"<br>";
	...
	*/
	...
	echo"<div style='background: #055293b3; position: absolute; overflow-y: auto; left: 1px; right: 1px; top: 1px; bottom: 1px; width: 1200px;	overflow-y: auto; overflow-x: auto;'>";	
		...
		echo"</div>";/*
		$FLETES_PROM	=libre_v1::forma_num($FLETES/$CONTADOR,2);
		$VIATICOS_PROM	=libre_v1::forma_num($VIATICOS/$CONTADOR,2);
		$DIESEL_PROM	=libre_v1::forma_num($DIESEL/$CONTADOR,2);
		$CASETAS_PROM	=libre_v1::forma_num($CASETAS/$CONTADOR,2);
		$FACTURAS_PROM	=libre_v1::forma_num($FACTURAS/$CONTADOR,2);
		$RYR_PROM		=libre_v1::forma_num($RYR/$CONTADOR,2);
		$GUIAS_PROM		=libre_v1::forma_num($GUIAS/$CONTADOR,2);
		$MANIOBRAS_PROM	=libre_v1::forma_num($MANIOBRAS/$CONTADOR,2);
		$VIAPASS_PROM	=libre_v1::forma_num($VIAPASS/$CONTADOR,2);*/
	...
	echo"</div>";
	/*
	echo"<div style='height: 210px; background: #132d9e99; position: absolute; top: 5px; left: 5px; right: 5px;'>";
		echo"<div style='position: absolute; left: 5px; top: 5px;'>";
		echo libre_v1::input2(text,'','',''					,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"Totales"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"Promedios"		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Fletes				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$FLETES			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$FLETES_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Viaticos			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$VIATICOS			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$VIATICOS_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Diesel				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$DIESEL			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$DIESEL_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Casetas			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$CASETAS			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$CASETAS_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Facturas			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$FACTURAS			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$FACTURAS_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',RYR				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$RYR				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$RYR_PROM			,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Guias				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$GUIAS				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$GUIAS_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',Maniobras			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$MANIOBRAS			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$MANIOBRAS_PROM	,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"Via Pass"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$VIAPASS			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',$VIAPASS_PROM		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo"</div>";
		echo"<div style='    position: absolute;top: 5px; bottom: 5px; left: 230px; background: rebeccapurple; '>";
		echo libre_v1::input2(text,'','',""					,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"Totales"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"Promedios"		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"Rendimiento "		,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',""					,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"rendimiento"		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"Flete r"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"total flete_r"	,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"ISR"				,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"total isr"		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"Netos"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"total netos "		,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"Abonos"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"total Abonos "	,'width: 70px; margin: .5px;');
		echo"<br>";
		echo libre_v1::input2(text,'','',"Para Chofer"			,'width: 70px; margin: .5px;');
		echo libre_v1::input2(text,'','',"total chofer "	,'width: 70px; margin: .5px;');
		echo"<br>";
		echo"</div>";
	echo"</div>";
	*/
	/*
		echo "<br>";
		while($datos=libre_v1::mysql_fe_ar	($consu5,$phpv)){
			$viaticos_con	= libre_v1::consulta	(viaticos,$conexion,ID_G,$datos[ID_G],'1','',$phpv,'');
			$viaticos		= libre_v1::mysql_fe_ar	($viaticos_con,$phpv);
			$casetas_con	= libre_v1::consulta	(casetas,$conexion,ID_G,$datos[ID_G],'1','',$phpv,'');
			$casetas		= libre_v1::mysql_fe_ar	($casetas_con,$phpv);
			$disel_con		= libre_v1::consulta	(disel,$conexion,ID_G,$datos[ID_G],'1','',$phpv,'');
			$disel			= libre_v1::mysql_fe_ar	($disel_con,$phpv);
			$guias_con		= libre_v1::consulta	(guias,$conexion,ID_G,$datos[ID_G],'1','',$phpv,'');
			$guias			= libre_v1::mysql_fe_ar	($guias_con,$phpv);
			$ryr_con		= libre_v1::consulta	(ryr,$conexion,ID_G,$datos[ID_G],'1','',$phpv,'');
			$ryr			= libre_v1::mysql_fe_ar	($ryr_con,$phpv);
			echo libre_v1::input2(text,'','',$datos[ID_G]		,'width: 70px; margin: .5px;');
			echo libre_v1::input2(text,'','',$datos[sueldo]		,'width: 70px; margin: .5px;');
			echo libre_v1::input2(text,'','',$viaticos[TOTAL2]	,'width: 70px; margin: .5px;');
			echo libre_v1::input2(text,'','',$casetas[TOTAL4]	,'width: 70px; 	margin: .5px;');
			echo libre_v1::input2(text,'','',$disel[TOTAL3]		,'width: 70px; 	margin: .5px;');
			echo libre_v1::input2(text,'','',$guias[TOTAL7]		,'width: 70px; 	margin: .5px;');
			echo libre_v1::input2(text,'','',$ryr[TOTAL6]		,'width: 70px; 	margin: .5px;');
			echo libre_v1::input2(text,'','',$datos[CLIENTE]	,'width: 70px; 	margin: .5px;');
			echo "<br>";
		}*/
	}
