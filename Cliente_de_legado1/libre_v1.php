...
if($libre_v1<>1)		{$libre_v1=1;	class libre_v1	{
		...
		function windows			($name,$style1,$style2,$contenido,$windows){
			...
			$conte=$conte.libre_v1::input2(button,$name,'',$actuador	,'position: absolute; right: 5px; width: 20px;',"act".$name,"onclick='windows(this,$name);'");//,'onclick="windows(this);"'
			
			$sub_style	="position: absolute; top: 17px; bottom: 2px; left: 2px; right: 2px; color: white; overflow: auto;";
			$libre="id='res$name' ";
			$conte		=$conte.libre_v1::div			($sub_style,$libre,$contenido);
			$libre="id='$name' class='div$name'";
			$res		=libre_v1::div					($style,$libre,$conte);
			return $res;
		}
	}
}
if($tablas_v1<>1)		{$tablas_v1=1;	class tablas_v1	{
		function info($db,$tb){
			$db1=empresa;
			$db2=login;
			if($db==$db1){
