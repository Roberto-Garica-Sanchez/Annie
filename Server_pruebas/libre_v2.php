...
if($libre_v2!=2)		{$libre_v2=2;	
	class libre_v2	{		
		...
		function consulta			($tabla,$conexion,$col_espe,$espe,$orde,$dire,$phpv,$buscar,$pre_sql)								{ 
			...
			if (($dire<>'')or($dire==desendente)or($dire==DESENDENTE))		$dire=DESC;
			if ($dire=='')		$dire=ASC;
		...
		function crea_array($name){//ID asignado entro del arrayA
			if($name=='')$array=array();//crea un array sin id
			if($name<>'')$array[$name]=array();//crear un arrayB dentro de otro arrayA
			return $array;
		}
		function add_array($array,$name,$new_dato){
			if($name=='')$array[]=$new_dato;//ingresa un dato al final del arrayA
			if($name=="==")$array[$new_dato][]=$new_dato;//esta mal  
			if($name<>'')$array[$name][]=$new_dato;
			return $array;
		}
