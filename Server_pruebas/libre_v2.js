...
/*
function busca_en_choferes(){
	elemento=chofer_busca.value;
	console.log("[busca_en_choferes]"+elemento);
	res=busca("botones_submenu","class");
	for(x=0; x<res.length; x++){
		//console.log(res[x]);
		if(res[x].value==elemento){
			console.log(res[x]);	
			list_choferes=res[x];
		}
	}
}
function busca(nombre,tipo){
	res="Ningun resultado" ;
	if(tipo=="class"){
		res=document.getElementsByClassName(nombre);
	}
	return res;
}*/