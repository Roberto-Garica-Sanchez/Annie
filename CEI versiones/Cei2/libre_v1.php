...
if($libre_v1<>1)		{$libre_v1=1;	
	class libre_v1	{		
		...
		function consulta			($tabla,$conexion,$col_espe,$espe,$orde,$dire,$phpv,$buscar,$pre_sql)								{ 
			...
			$consu=libre_v1::ejecuta($conexion,$consulta,$phpv);
			return $consu;
		}
		...
		function despliegre_mysql	($name,$name2,$consu,$descarga,$phpv,$libre,$className,$dataset,$setelemento)												{
			$res=$consu;
			...
			if($name2<>'')$d=$d."<OPTION value='$name'>$name2</OPTION>";
				...
				while($datos= libre_v1::mysql_fe_ar($res,$phpv)){
					...
					if($dataset<>""){						
						if($datos[$dataset]==$setelemento){$set='selected';}
					}
