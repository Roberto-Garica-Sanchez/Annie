...
function windows(carga,respuesta,opcion,consola,res,elemento,carga_aux){
		...
		if((opcion=="entradas")||(opcion=="salida")){
			...
			if(respuesta=="1"){
				var  send=
					...
					;
				if(opcion=="entradas"){
					consulta("des_data_alum",send,res_entrada,res_entrada,"inn","inn");
					consulta("ver_cierres",send,res_entrada,res_entrada,"inn","inn",windows,opcion);
				}
				if(opcion=="salida"){
					consulta("des_data_alum",send,res_salida,res_salida,"inn","inn");
					consulta("ver_cierres",send,res_salida,res_salida,"inn","inn",windows,opcion);
				}
			}
		}
		if(opcion=="ver_cuenta"){
			if(!elemento){
				if(respuesta=="0"){//Cuenta disponible para guardar 
					consola.innerHTML="The Account Available";
					cuenta.style.background="";
					cuenta.style.color="";
					
				}
				if(respuesta=="1"){//el numero de cuenta ya existe 
					consola.innerHTML="The Account Exists";
					cuenta.style.background="pink";
					cuenta.style.color="black";
	...
	if(carga=="reg_alum"){				//resive la respuesta de guardar alumnos
		...
		if(opcion=="account_studen"){
			...
			if(respuesta=="1"){						
				...
				consola.innerHTML="Save Student";	
	...
	if(carga=="elimina_studen")	{
		...
		if(respuesta=="1"){
			consola.innerHTML="Delete Studen";	
			name_studen.value="";
