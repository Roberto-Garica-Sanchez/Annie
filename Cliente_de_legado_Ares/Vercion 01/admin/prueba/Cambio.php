...
$t_d_g= round($total3+$t_g+$comi+$total4_via,2);
...
if($_POST['menu-list']==$listn5){
	...
	$d=input(hidden,presio_d,$_POST[presio_d]).input($type2,presio_d,$_POST[presio_d]);
    $final="
...
if($_POST['menu-list']==$listn11){
	$id='';							$style="position: absolute; left: 355px; top: 0px; background: rgba(0, 0, 0, 0.77) none repeat scroll 0% 0%; color: white;";
...
//-----------------------
	$id='';							$style="position: absolute; left: 110px; top: 0px; background: rgba(0, 0, 0, 0.77) none repeat scroll 0% 0%; color: white;";
	$style_t="background: black";	$title=Arrastrados;
...
$i2=2;                  $d2=input(hidden,Carta2,$_POST[Carta2]).input($type2,Carta2,$_POST[Carta2],'',"color: $indc2;",'maxlength="4"');
$i3=3;                  $d3=input(hidden,Carta3,$_POST[Carta3]).input($type2,Carta3,$_POST[Carta3],'',"color: $indc3;",'maxlength="4"');
$i4=4;                  $d4=input(hidden,Carta3,$_POST[Carta3]).input($type2,Carta4,$_POST[Carta4],'',"color: $indc4;",'maxlength="4"');
...
$i10='Flete R.';        $d10=input(hidden,flete_r,$_POST[flete_r]).input($type2,flete_r,$_POST[flete_r],'','','maxlength="10"');
$i11="Kms inicio";      $d11=input(hidden,km_i,$_POST[km_i]).input($type2,km_i,$_POST[km_i],'','','maxlength="10"');
$i12="Kms fin";         $d12=input(hidden,km_f,$_POST[km_f]).input($type2,km_f,$_POST[km_f],'','','');
...
IF ($grava==0)$d20="Verifique <br> marcadas en <br>Rosa o Vacias";
if($_POST[este]==button){$este=MODIFICAR;$d20='';}
if($_POST[este]==text){$este=CERRAR;}
$d16=input(submit,cam,$este);
$d17=input(submit,accion,NUEVO);
...
$ts="background: black;";
$title1='Generales';
$i1="Gastos T.";   	$d1=input(button,'',$g_t,"Casetas[$total4] + Facturas[$total5] + RYR[$total6] + Guias[$total7] + Maniobras[$total8] =$g_t",'background: #FDFD96; color: black;');
...
$i5='';				$d5=input(button,'',$dif1,"+[$total2]Viaticos\r+[$rete]Retencion\r -[$g_t]Gasto T.\r -[$c]chofer\r----------------\r[$dif1] Total ","background: $sd5; color: $ld5;");
$tc6='#989898';			$i6='';			$d6='';
$i7='Sin. Cho.';	$d7=input(button,'',$g_t,"Casetas[$total4] + Facturas[$total5] + RYR[$total6] + Guias[$total7] + Maniobras[$total8] =$g_t",'background: #FDFD96; color: black; ');
$i8=Viaticos;		$d8=input(button,'',$total2,'','background: greenyellow; color: black;');
$i9='';				$d9=input(button,'',$dif2,"+Viaticos[$total2] -Gasto T.[$g_t] = $dif2 ","background: $sd9; color: $ld9;");
$tc10='#989898';	$i10='';			$d10='';
$i11=Flete;			$d11=input(button,'',$_POST[flete_r],'','background: aqua;');
$i12='Total G.';	$d12=input(button,'',$t_d_g,"Via Pass [$total4_via] +Diesel[$total3] +Gasto T.[$t_g] + Comision[$comi] = $t_d_g ","background: orange; color: black;");
...
$i16='Actual';		$d16=input(button,'',$dif1,"+[$total2]Viaticos\r -[$rete]Retencion\r -[$g_t]Gasto T.\r -[$c]chofer\r----------------\r[$dif1] Total ","background: $indf16; color: $indc16;");
$i17='Arrastado';	$d17=input(button,'',$total10,"Suma de todas las Cuentas Arastradas","background: $indf17; color: $indc17;");
$i18='';			$d18=input(button,'',$dif3,"Actual[$dif1] + Arrastrada[$total10]= $dif3","background: $indf18; color: $indc18;");
$i19=Abonos;		$d19=input (button,'',$total9,"Suma de los abonos de esta cuenta ","background: $indf19; color: $indc19;");
$i20=Total;			$d20=input(button,'',$dif4,"Arrastrado[$total10]-Abonos[$total9]=$dif4","background: $indf20; color: $indc20;");
include('tablero.php');
...
echo"<TEXTAREA style='width: 100px; height: 220px; color: white; background: rgba(0, 0, 0, 0.77) none repeat scroll 0% 0%;'>";
	...
	include('espe_tab.php');
	IF (($grava==1)&&($_POST[Grava]==Guardar)){MYSQL_QUERY($res,$conexion)or die("Error <br>$res"); echo"\r kilometrajes Guardados ";}
	//-----------------------------------------------------------
	...
	IF (($grava==1)&&($_POST[Grava]==Guardar)){MYSQL_QUERY($res,$conexion)or die("Error <br>$res");echo"\r Fletes Guardados ";}
	//----------------------------------------------------------
			...
			if ($c2<>''){
				...
				if ($c2<>$c1){
					...
					IF (($grava==1)&&($_POST[Grava]==Guardar)&&($fec_fin>=20160601)){
						echo"\r".$res;
						MYSQL_QUERY($res,$conexion)or die("Error \r$res\r");
					}
				}
			}
		}	
