...
if($_POST[Carta0]==""){$_POST[Carta0]=$_POST[Carta1];}
...
$consu_folio	= 	libre_v1::consulta	(folio,$conexion	,'','','','1'	,$phpv,'');
$consu_abo_acu	= 	libre_v1::consulta	(abo_acu,$conexion	,'','','','1'	,$phpv,'');
...
$N_fact			=	libre_v2::compro($_POST[chofer],Nombre_Ch	,ulti_viaje	,$consu_choferes,$conexion,$phpv);
$revi			=	libre_v2::compro($_POST[Carta1],ID_G		,Revisado,$consu_folio,$conexion,$phpv);
if($_POST[ID_ac1]<>"")$estado_arr		=	libre_v2::compro($_POST[ID_ac1],ID_G		,add_en,$consu_abo_acu,$conexion,$phpv);
...
if($_POST[name2set]==Nuevo){	//proceso de guardado
	...
	if($_POST[operador]=="Guardar"){
		$grava=true;
		$open_consola=false;
		if($_POST[Carta1]<>"")$consu_carta1	=libre_v2::compro($_POST[Carta1],ID_G		,'',$consu_folio,$conexion,$phpv);	
		if($_POST[Carta2]<>"")$consu_carta2	=libre_v2::compro($_POST[Carta2],ID_G		,'',$consu_folio,$conexion,$phpv);
		if($_POST[Carta3]<>"")$consu_carta3	=libre_v2::compro($_POST[Carta3],ID_G		,'',$consu_folio,$conexion,$phpv);
		if($_POST[Carta4]<>"")$consu_carta4	=libre_v2::compro($_POST[Carta4],ID_G		,'',$consu_folio,$conexion,$phpv);
		...
		//if (($_POST[km_i]>=$_POST[km_f])or($total_km<=200))				{//que este kilometraje correcto 
...
if($_POST[name2set]==Modificar){//proceso de modificado
	...
	if($_POST[operador]=="Guardar"){
		...
		if($_POST[Carta1]<>""){
			...
			$datos=libre_v1::mysql_fe_ar($res,$phpv);
...
if($_POST[CambRevi]=="")$_POST[CambRevi]=0;
if($revi==0){$estado=Pendiente;$_POST[CambRevi]=$revi; $style_estado="background: pink; color: black;";}
if($revi==''){$estado=Pendiente;$_POST[CambRevi]=0; $style_estado="background: pink; color: black;";}
if($revi==1){$estado=Revisado;$_POST[CambRevi]=$revi; $style_estado="background: Greenyellow; color: black;";}
//if($dif4<0)				{$style_dif4="background: pink; color: black;";}else 	{$style_dif4="background: Greenyellow; color: black;";}
...
//--------------------------------operadores2
	...
	if($_POST[name2set]==Nuevo){	
		if(($_POST[operador]=="Guardar")and($grava==true)){
			include("../Guarda.php");
...
echo"<div id='general_info' style='background: #05486cab;width: 220px;right: 0px;top: 0px;bottom: 5px;position: absolute;color: white;'>";
	echo"<div id='datos_info'>";
		...
		echo libre_v1::input2(button,estado,'',$estado				,$style_estado."width: 60%",'',"disabled",botones_submenu).libre_v1::input2(hidden,'CambRevi','',$_POST[CambRevi],"",'',"");
		...
		*/
		...
		echo libre_v1::input2(hidden,'Carta0','',$_POST[Carta0]		,"width: 40%"				,''		,"",botones_submenu);
		...
		echo libre_v1::input2(text,'n_c','',$_POST[n_c]			,"width: 60%"				,'n_c'		,"readonly=''",botones_submenu);
		///echo libre_v1::input2(text,'','','$N_fact'			,"width: 60%"				,'',"readonly=''",botone_n);
...
echo "<div id='general_datos' style='position: absolute;right: 220px;background: #444;padding: 5px;top: 0px; box-shadow: inset 0px 0px 5px black;'>";
	...
	echo libre_v1::input2($TYPE,'','',TOTALES,$style_all,'','disabled').libre_v1::input2(hidden,hidden_fe				,'',5	);
...
//$style_all=$style_all."background: #343434; color: #0075ff;";
echo "<div id='gene_actual' style='$style_gene_actual position: absolute;background: #28336985;padding: 5px;bottom: 0px;right: 220px;'>";
	...
	echo libre_v1::input2($TYPE,'sueldo','',$comicion				,$style_all,"CHOFER",'readonly=""');	
	...
	echo libre_v1::input2($TYPE,'',"Diesel[".$_POST[TOTAL3]."] + Gasto T.[$t_g] + Chofer Neto[$suedo_chofer] + Via pass[".$_POST[TOTAL9]."] = $T_G_F ",$t_d_g,$style_all2."background: orange;color: black;",T_G_F,'disabled');	
	echo "<br>";
	echo libre_v1::input2($TYPE,'','',"Descuento ISR"					,$style_all,'','disabled');
	echo libre_v1::input2($TYPE,'isr','',$rete					,$style_all,ISR,'readonly=""');
	...
	echo "<br>";
	echo libre_v1::input2($TYPE,'','','Chofer Neto'			,$style_all,'','disabled');	
	echo libre_v1::input2($TYPE,suedo_chofer,'',$suedo_chofer			,$style_all."background: white; color: black;",'readonly="readonly"');	
	echo libre_v1::input2($TYPE,'','',''					,$style_all,'','disabled');	
	echo libre_v1::input2($TYPE,'','',''					,$style_all,'','disabled');	
	echo libre_v1::input2($TYPE,'','',''					,$style_all,'','disabled');	
	echo libre_v1::input2($TYPE,'','',''					,$style_all,'','disabled');	
...
echo "<div id='gene_combustible' style='$style_gene_combustible    position: absolute;background: #444444;padding: 5px;background: #28336985;top: 464px;right: 836px;'>";
	...
	echo libre_v1::input2($TYPE,presio_d,'',$_POST[presio_d],$style_all."background: white; color: black;box-shadow: inset 0px 0px 8px #0072fd;",presio_d,"placeholder='Costo' onkeyup='calcula_update();calcu_combustible();'onkeypress='return valida_n(event,crome_i)' maxlength='10'");				
	...
	echo "<br>";	
	echo libre_v1::input2($TYPE,'','','Inicio'				,$style_all				,''		,"disabled");
	echo libre_v1::input2($TYPE,crome_i,'',$_POST[crome_i]	,$style_all.'background: white;color: black;box-shadow: inset 0px 0px 8px #0072fd;'	,crome_i,"onkeyup='calcula_update();calcu_combustible();'onkeypress='return valida_n(event,crome_f)' maxlength='15'");	
	...
	echo"<br>";
	echo libre_v1::input2($TYPE,'','','Final'				,$style_all				,''		,"disabled");
	echo libre_v1::input2($TYPE,crome_f,'',$_POST[crome_f]	,$style_all.'background: white;color: black;box-shadow: inset 0px 0px 8px #0072fd;'	,crome_f,"onkeyup='calcula_update();calcu_combustible();'onkeypress='return valida_n(event)' maxlength='15'");
	echo libre_v1::input2($TYPE,'','',' Recoridos',$style_all,'',"disabled");		
	echo libre_v1::input2($TYPE,'','',$dif_km." Km",$style_all,Total_km,"disabled");
	echo "<br>";
	echo libre_v1::input2($TYPE,'','','Despachado Lts'				,$style_all				,''		,"disabled");
	echo libre_v1::input2($TYPE,crome_t,'',$dif_despacho,$style_all.'color: #a4c1e3;'	,'crome_t',"disabled");
...
echo "<div id='gene_arrastre' style='$style_gene_arrastre left: 100px;position: relative;background: #28336985;padding: 5px;float: left;'>";		
	...
	echo libre_v1::input2($TYPE,'','','',$style_all,'',"disabled");
	echo "<br>";		
	echo libre_v1::input2(text,ID_ac1	,'Click Para Eliminar',$_POST[ID_ac1]	,$style_all.'background: #343434; color: #0075ff;','ID_ac1'		,"placeholder='Folio add' readonly='readonly' onclick='elimi_arra(this);calcula_update();' ");
	//echo libre_v1::input2($TYPE,ac1		,'',$_POST[ac1]		,$style_all.$style_ID_ac1.'background: white;box-shadow: inset 0px 0px 8px #0073ff;','ac1'						,"onkeyup='calcula_update();'onchange='calcula_update();' readonly='readonly'");
	echo libre_v1::input2($TYPE,ac1		,'',$_POST[ac1]		,$style_all.$style_ID_ac1,'ac1'						,"onkeyup='calcula_update();'onchange='calcula_update();' readonly='readonly'");
	echo "<br>";
	...
	echo libre_v1::input2($TYPE,'dif1','',$dif1,$style_all.$style_dif1,DIF_TV2,$libre_all);
	echo "<br>";		
...
$id=Consola;
if($consola=="open"){$class=$id;}else{$class="min";}
