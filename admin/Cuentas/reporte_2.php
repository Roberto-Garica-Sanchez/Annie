...
//contenedor principal 
	...
	if($_POST[$menu]==$listn1){ 
		...
		$consu0=consulta(fechas,$conexion,'','',ID_G,1,$phpv);
		mysql_da_se($consu0,0,$phpv);
		$flete="";
		while($dato2=mysql_fe_ar($consu0,$phpv)){	
			$A=$dato2[A];$M=$dato2[M];$D=$dato2[D];
			$fec_set =$A.$M.$D;
			$fecha	= "$D-$M-$A";
			if (($fec_ini<=$fec_set)&&($fec_fin>=$fec_set)){
				$consu=consulta(folio,$conexion,ID_G,$dato2[ID_G],ID_G,1,$phpv);$datos=mysql_fe_ar($consu,$phpv);
				if(	
